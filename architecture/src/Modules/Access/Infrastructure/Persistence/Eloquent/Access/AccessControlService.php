...
class AccessControlService
{
    ...
    public function transferOwnership(Model $workspace, Model $resource, Model $from, Model $to): void
    {
        ...
        if ($resource instanceof Media) {
            $resource->forceFill(['uploaded_by' => $to->getKey()])->save();
        } elseif ($resource instanceof Folder) {
            $resource->forceFill(['created_by' => $to->getKey()])->save();
        }
    ...
    private function isOwner(Model $resource, Model $user): bool
    {
        if ($resource instanceof Folder) {
            return (int) ($resource->created_by ?? 0) === (int) $user->getKey();
        }

        if ($resource instanceof Media) {
            return (int) ($resource->uploaded_by ?? 0) === (int) $user->getKey();
        }
    ...
    {
        $scope = $resource->getAttribute('access_scope');
