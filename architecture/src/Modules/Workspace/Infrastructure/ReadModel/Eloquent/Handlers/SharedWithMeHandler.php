...
final class SharedWithMeHandler
{
    ...
    public function handle(SharedWithMe $query): array
    {
        ...
        $folders = Folder::query()
            ...
            ->orderByDesc($this->latestGrantTimestamp('folders.id', $folderMorph, $workspace->getKey(), $actor->getKey()));
        ...
        $media = Media::query()
            ...
            ->whereExists(fn ($grant) => $grant
                ...
                ->where('collaborator_grants.workspace_id', $workspace->getKey())
                ->where('collaborator_grants.user_id', $actor->getKey()))
            ->orderByDesc($this->latestGrantTimestamp('media.id', $mediaMorph, $workspace->getKey(), $actor->getKey()));
