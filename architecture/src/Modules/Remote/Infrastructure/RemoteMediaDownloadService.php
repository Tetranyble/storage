...
class RemoteMediaDownloadService
{
    ...
    public function download(
        ...
        $maxSize = $this->resolveMaximumSize($maxSizeBytes);
    ...
    private function resolveMaximumSize(?int $requestedMaxSize): int
    {
        $uploadMax = max(1, (int) config(
            'tetranyble-storage.uploads.max_size',
            50 * 1024 * 1024,
        ));
        $remoteMax = (int) config(
            'tetranyble-storage.remote.max_size',
            50 * 1024 * 1024,
        );

        $effectiveRemoteMax = $requestedMaxSize ?? $remoteMax;
        if ($effectiveRemoteMax <= 0) {
            return $uploadMax;
        }

        return min($uploadMax, $effectiveRemoteMax);
    }
