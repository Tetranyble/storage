...
final class LaravelStorageEventPublisher implements StorageEventPublisher
{
    ...
    { if (! $value instanceof Folder) throw new \InvalidArgumentException('Expected Folder model.'); return $value; }
    private function media(object $value): Media
    { if (! $value instanceof Media) throw new \InvalidArgumentException('Expected Media model.'); return $value; }
    private function share(object $value): MediaShare
    { if (! $value instanceof MediaShare) throw new \InvalidArgumentException('Expected MediaShare model.'); return $value; }
    private function actor(?object $value): ?Model
    { if ($value === null) return null; if (! $value instanceof Model) throw new \InvalidArgumentException('Expected Eloquent actor model.'); return $value; }
}
