...
`tetranyble/storage` uses GitHub Actions as the authoritative CI/release pipeline and CircleCI as an independent verification-only CI fallback. Only GitHub Actions may create version tags, GitHub Releases, or drive Packagist publication verification.
...
## CircleCI verification fallback

`.circleci/config.yml` mirrors the release-critical test surface without any publication credentials or release commands. It runs the production gate, PHP/Laravel compatibility, PostgreSQL/MySQL integration and query-performance lanes, and the MinIO S3 contract suite. `composer run architecture:circleci` prevents CircleCI from gaining tag/release/Packagist publication authority.

If GitHub Actions is unavailable because of an account-level billing lock, connect the public repository to CircleCI and use **CircleCI Required CI** as the temporary required branch check after its first run. Do not treat CircleCI as a release publisher; see `docs/CIRCLECI.md` for setup and policy.
