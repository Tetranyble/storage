...
foreach ([
    ...
    'docs/PERFORMANCE_AND_INDEXES.md', 'docs/STEP_12_RELEASE_HARDENING.md', 'docs/CI_CD.md', 'docs/CIRCLECI.md', '.env.example', '.gitattributes',
    '.github/workflows/ci.yml', '.github/workflows/release.yml', '.github/dependabot.yml', '.circleci/config.yml',
    'scripts/check-circleci-integration.php', 'scripts/validate-release-version.php', 'scripts/verify-packagist-release.php',
] as $file) {
...
$circleCi = $read('.circleci/config.yml');
foreach ([
    'composer run production:gate', 'composer audit --locked', 'postgres:17', 'mysql:8.4',
    'quay.io/minio/minio:latest', 'composer benchmark:queries', 'name: CircleCI Required CI',
] as $needle) {
    if (! str_contains($circleCi, $needle)) {
        $fail("CircleCI release verification coverage is missing [{$needle}].");
    }
}
foreach (['git push ', 'git tag -a', 'gh release', 'verify-packagist-release.php', 'packagist.org/api/'] as $needle) {
    if (str_contains($circleCi, $needle)) {
        $fail("CircleCI must remain verification-only; publication marker [{$needle}] is forbidden.");
    }
}
$ok('CircleCI mirrors release-critical verification while publication remains GitHub-only.');
