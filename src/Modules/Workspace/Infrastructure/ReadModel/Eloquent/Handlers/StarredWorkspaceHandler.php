    ...
    public function handle(StarredWorkspace $query): array
    {
        ...
        $folders = Folder::query()
            ...
            ->whereExists(fn ($star) => $star
                ...
                ->where('resource_stars.workspace_id', $workspace->id)
                ->where('resource_stars.user_id', $actor->id))
            ->orderByDesc($this->latestStarTimestamp('folders.id', $folderMorph, $workspace->id, $actor->id));
        ...
        $media = Media::query()
            ...
            ->whereExists(fn ($star) => $star
                ...
                ->where('resource_stars.workspace_id', $workspace->id)
                ->where('resource_stars.user_id', $actor->id))
            ->orderByDesc($this->latestStarTimestamp('media.id', $mediaMorph, $workspace->id, $actor->id));
