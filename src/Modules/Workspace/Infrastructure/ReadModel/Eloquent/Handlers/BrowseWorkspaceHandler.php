...
final class BrowseWorkspaceHandler
{
    ...
    public function handle(BrowseWorkspace $query): array
    {
        ...
        $folderQuery = Folder::query()->where('workspace_id', $workspace->id)->where('parent_id', $current->id)
        ...
        $filesQuery = Media::query()->where('workspace_id', $workspace->id)->whereNull('deleted_at');
        if ($current->is_root) {
    ...
    {
        return Folder::firstOrCreate(['workspace_id' => $workspace->id, 'is_root' => true], [
            'name' => $workspace->name, 'slug' => Str::slug($workspace->name.'-root'), 'path' => 'root',
    ...
    private function resolveFolder(Model $workspace, string $relativePath): ?Folder
    {
        ...
        return Folder::query()->where('workspace_id', $workspace->id)
