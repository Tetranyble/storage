...
class MediaRelocationService
{
    ...
    public function rename(Media $media, string $name, ?Model $actor = null): Media
    {
        $trimmed = trim($name);
        if ($trimmed === '') {
            throw new InvalidStorageOperationException('File name cannot be empty.');
        }

        $disk = $media->disk instanceof Disk ? $media->disk : StorageConfig::defaultDisk();
        ...
        $plans = $this->relocationPlans($media, $disk, $oldPath, $newPath);
        $attempted = [];

        try {
            $attempted = $this->copyPlans($plans, $media);

            DB::transaction(function () use ($media, $newPath, $newFilename, $trimmed): void {
                $media->forceFill([
                    'path' => $newPath,
    ...
    public function move(Media $media, Folder $targetFolder, ?Model $actor = null): Media
    {
        ...
        try {
            ...
            DB::transaction(function () use ($media, $newPath, $targetFolder): void {
                $media->forceFill([
                    'path' => $newPath,
