...
class MediaService implements RemoteMediaImporter
{
    ...
    /**
     * Register a provider-direct object whose quota was reserved when its
     * direct-upload session was created. This deliberately skips lifecycle
     * quota reservation; the session reservation becomes normal Media usage.
     */
    public function registerDirectUploadObject(
        string $storedPath,
        int $size,
        ?string $mime,
        ?string $originalName,
        ...
        $existing = Media::query()->where('direct_upload_session_uuid', $sessionUuid)->first();
        if ($existing instanceof Media) {
            return $existing;
        }

        $directOptions = new MediaUploadOptions(
            model: $options->model,
            ...
            originalName: $originalName ?? $options->originalName,
            ...
            auditMeta: array_merge($options->auditMeta, ['direct_upload_session_uuid' => $sessionUuid]),
            directUploadSessionUuid: $sessionUuid,
        );
        ...
        return $this->finishStoredFilePersistence($state, $directOptions);
    }
    ...
    public function uploadFor(
        ...
        $options = MediaUploadOptions::forModel(
            model: $model, workspaceId: StorageConfig::actorWorkspaceId($model),
            purpose: $purpose,
            directory: $directory,
            disk: $disk,
            module: $directory,
    ...
    public function attachExternalFor(
        ...
        $disk ??= $this->paths->diskForExternalUrl($url);
        ...
        $folder = $workspace ? $this->paths->folderForOptions($workspace, MediaUploadOptions::forModel($model, $purpose)) : null;
    ...
    public function restoreRevision(Media $revision, ?int $userId = null): Media
    {
        ...
        $disk = $revision->disk instanceof Disk ? $revision->disk : $this->paths->disk($options);

        if ($revision->path && ! $this->isExternalUrl($revision->path) && $disk instanceof Disk) {
            $targetDirectory = $this->paths->uploadDirectory($options, $workspace);
            $filename = $this->paths->storedFilename(
    ...
    public function attachExistingMediaToModel(
        ...
        try {
            ...
            if ($hasStoredObject && $disk) {
                $newPath = trim($this->paths->uploadDirectory($options, $workspace).'/'.basename((string) $oldPath), '/');
                if ($newPath !== $oldPath) {
            ...
            $folder = $workspace ? $this->paths->folderForOptions($workspace, $options) : null;
    ...
    protected function assertConfiguredUploadSize(int $size): void
    {
    ...
    protected function persistUploadedFile(UploadedFile $file, MediaUploadOptions $options): Media
    {
        ...
        $this->assertConfiguredUploadSize($size);
        $mime = $file->getClientMimeType() ?: $file->getMimeType();
        $originalName = $options->originalName ?? $file->getClientOriginalName();
        $checksum = $this->checksumForPath($file->getRealPath());
        $targetDirectory = $this->paths->uploadDirectory($options, $workspace);
        $storedFilename = $this->paths->storedFilename($originalName, $options->preserveFilename);
    ...
    protected function persistPathUpload(string $path, MediaUploadOptions $options): Media
    {
        ...
        try {
            ...
            $this->assertConfiguredUploadSize((int) $size);
    ...
    protected function finishStoredFilePersistence(array $state, MediaUploadOptions $options): Media
    {
        ...
        if ($options->dispatchPostProcessing
            || (bool) config('tetranyble-storage.processing.auto_dispatch', true)
    ...
    protected function versionUploadOptions(Media $media, ?int $userId = null): MediaUploadOptions
    {
        ...
        if ($media->mediable_type && $media->mediable_id) {
            $modelClass = Relation::getMorphedModel($media->mediable_type) ?? $media->mediable_type;
            $model = is_a($modelClass, Model::class, true)
                ? $modelClass::query()->find($media->mediable_id)
                : null;
