...
class MediaVersioningService
{
    ...
    public function deleteVersion(Model $workspace, Media $version, Model $actor): void
    {
        ...
        $group = new VersionGroup(
            max(0, (int) $version->version_number), max(1, (int) $version->version_number + 1),
            $version->current ? $version->getKey() : null,
        );
        $group->assertVersionCanBeDeleted((bool) $version->current, $groupCount);
    ...
    private function reserveNextVersionNumber(string $groupUuid): int
    {
        return DB::transaction(function () use ($groupUuid): int {
            ...
            $state = new VersionGroup(
                $observedMax, (int) $group->next_version_number, $group->current_media_id ?? null,
            );
            $next = $state->reserveNextVersion();
